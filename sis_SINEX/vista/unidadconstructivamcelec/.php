<?php

header("content-type: text/javascript; charset=UTF-8");
?>
<script>
Phx.vista.ucmcelecotrosgastos=Ext.extend(Phx.gridInterfaz,{

	constructor:function(config){
		this.maestro=config.maestro;
    	//llama al constructor de la clase padre
		Phx.vista.ucmcelecotrosgastos.superclass.constructor.call(this,config);
		this.init();
		this.load({params:{start:0, limit:this.tam_pag}})
	},
			
	Atributos:[
		{
			//configuracion del componente
			config:{
					labelSeparator:'',
					inputType:'hidden',
					name: 'id_unidadconstructivamcelec'
			},
			type:'Field',
			form:true 
		},
		{
			//configuracion del componente
			config:{
					labelSeparator:'',
					inputType:'hidden',
					name: 'id_item'
			},
			type:'Field',
			form:true 
		},
		{
			//configuracion del componente
			config:{
					labelSeparator:'',
					inputType:'hidden',
					name: 'id_otrauc'
			},
			type:'Field',
			form:true 
		},	
		{
			config:{
				name: 'otrosgastos',
				fieldLabel: 'Otros Gastos',
				allowBlank: true,
				anchor: '80%',
				gwidth: 100,
				maxLength:50
			},
				type:'TextField',
				filters:{pfiltro:'otrosgastos',type:'string'},
				id_grupo:1,
				grid:true,
				form:false
		},		
		{
			config:{
				name: 'cantidadog',
				fieldLabel: 'Cantida dog',
				allowBlank: true,
				anchor: '80%',
				gwidth: 100,
				maxLength:1179650
			},
				type:'NumberField',
				filters:{pfiltro:'cantidadog',type:'numeric'},
				id_grupo:1,
				grid:true,
				form:false
		},
		{
			config:{
				name: 'valorunitario',
				fieldLabel: 'Valor Unitario',
				allowBlank: false,
				anchor: '80%',
				gwidth: 100,
				maxLength:1179652
			},
				type:'NumberField',
				filters:{pfiltro:'valorunitario',type:'numeric'},
				id_grupo:1,
				grid:true,
				form:false
		},
		{
			config:{
				name: 'valorog',
				fieldLabel: 'Valor og',
				allowBlank: true,
				anchor: '80%',
				gwidth: 100,
				maxLength:1179650
			},
				type:'NumberField',
				filters:{pfiltro:'valorog',type:'numeric'},
				id_grupo:1,
				grid:true,
				form:false
		}
	],
	tam_pag:50,	
	title:'Otros Gastos y Resumen General',
	ActList:'../../sis_SINEX/control/Unidadconstructivamcelec/listarOtrosgastos',
	id_store:'id_descripcion',
	fields: [
		{name:'id_item', type: 'numeric'},
		{name:'id_otrauc', type: 'numeric'},
		{name:'otrosgastos', type: 'string'},		
		{name:'cantidadog', type: 'numeric'},
		{name:'valorunitario', type: 'numeric'},		
		{name:'valorog', type: 'numeric'},
	],
	sortInfo:{
		field: 'id_descripcion',
		direction: 'ASC'
	},
	bdel:false,
	bsave:false,
	
	onReloadPage:function(m){
		this.maestro=m;
		this.store.baseParams={id_unidadconstructivamcelec:this.maestro.id_unidadconstructivamcelec};
		this.load({params:{start:0, limit:50}})
	},
	loadValoresIniciales:function()
	{
		Phx.vista.ucmcelecotrosgastos.superclass.loadValoresIniciales.call(this);
		this.getComponente('id_unidadconstructivamcelec').setValue(this.maestro.id_unidadconstructivamcelec);
	}
	}
)
</script>
		
		